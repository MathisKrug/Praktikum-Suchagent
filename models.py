...
class Job:
    ...
    # Nachweis. 'posted' ist das Ausschreibungsdatum laut Quelle - nicht das
    # Datum, an dem wir die Anzeige gesehen haben. Wenn die Quelle keines
    # liefert, bleibt das Feld leer und wird als "nicht auffindbar" angezeigt.
    # Niemals das Erfassungsdatum hier hineinschreiben.
    posted: str = ""
    source: str = ""
    ...
    def uid(self) -> str:
        ...
        Bewusst NICHT die URL: verschiedene Quellen liefern dieselbe Anzeige mit
        unterschiedlichen Weiterleitungs-URLs, und eine URL-basierte ID haelt
        die beiden faelschlich fuer zwei verschiedene Stellen. Bleibt wichtig,
        auch wenn aktuell nur ein Aggregator laeuft - eine Anzeige kann ueber
        Adzuna und ueber die Karriereseite gleichzeitig hereinkommen.
        """
        basis = "|".join([
            _fold(self.company),
            _fold(self.title),
