...
def _ampel(h: dict) -> str:
    """gruen = liefert, gelb = still, rot = Fehler oder lange still."""
    if h.get("last_error"):
        return "rot"
    quiet = h.get("quiet_runs") or 0
    if quiet >= 7:
        return "rot"
    if quiet >= 1:
        return "gelb"
...
def render_html(jobs: list[dict], errors: list[str], out: str = "docs/index.html",
    ...
    for j in jobs:
        payload.append({
            ...
            # Nachweis. Leeres Ausschreibungsdatum wird als solches angezeigt -
            # niemals durch das Erfassungsdatum ersetzt.
            "source": j.get("source") or "Quelle nicht vermerkt",
            "posted": (j.get("posted") or "").strip(),
        })
    ...
    health_payload = [{
        ...
        "kind": h.get("kind") or "",
        "state": _ampel(h),
        "count": h.get("last_count") or 0,
        "quiet": h.get("quiet_runs") or 0,
        "error": (h.get("last_error") or "")[:200],
        "last_ok": (h.get("last_ok") or "")[:10],
    } for h in health]
    health_payload.sort(key=lambda h: ({"rot": 0, "gelb": 1, "gruen": 2}[h["state"]],
                                       h["source"]))
    n_rot = sum(1 for h in health_payload if h["state"] == "rot")
    n_gelb = sum(1 for h in health_payload if h["state"] == "gelb")
...
<style>
  ...
  * {{ box-sizing:border-box; -webkit-tap-highlight-color:transparent; }}
  ...
  .src {{ font-size:11px; color:var(--dim); margin-top:7px;
          border-top:1px solid var(--line); padding-top:7px; }}
  .dot {{ display:inline-block; width:9px; height:9px; border-radius:50%;
          margin-right:8px; flex-shrink:0; }}
  .dot.gruen {{ background:#2ecc71; }}
  .dot.gelb  {{ background:#e2b93b; }}
  .dot.rot   {{ background:#e5544b; }}
</style>
</head>
<body>
<header>
  ...
  <div class="tabs">
    ...
    <button data-v="quellen">Quellen{' &middot; ' + str(n_rot) + ' rot' if n_rot else ''}</button>
  </div>
</header>
...
const HEALTH = {json.dumps(health_payload, ensure_ascii=False)};
...
function render(filter) {{
  ...
  list.innerHTML = rows.map(j => {{
    ...
    return '<a class="job" href="' + esc(j.url) + '" target="_blank" rel="noopener">'
      ...
      + '<div class="why">' + esc(j.reasons) + '</div>'
      + '<div class="src">Quelle: ' + esc(j.source) + ' &middot; ausgeschrieben: '
      + (j.posted ? esc(j.posted) : 'nicht auffindbar')
      + ' &middot; erfasst: ' + esc(j.first_seen) + '</div></a>';
  }}).join('');
}}

function renderHealth() {{
  if (!HEALTH.length) {{
    list.innerHTML = '<div class="empty">Noch keine Quellendaten.<br>'
      + 'Erscheint nach dem naechsten Lauf.</div>';
    return;
  }}
  list.innerHTML = '<div class="note">Gruen: liefert. Gelb: laeuft durch, liefert '
    + 'aber nichts. Rot: Fehler oder seit sieben Laeufen still. '
    + 'Eine stille Quelle ist gefaehrlicher als eine kaputte &ndash; sie sieht '
    + 'im Log wie Erfolg aus.</div>'
    + HEALTH.map(h => {{
    const bits = [];
    bits.push('<span class="tag">' + h.count + ' Stellen zuletzt</span>');
    if (h.quiet) bits.push('<span class="tag">' + h.quiet + ' Laeufe still</span>');
    if (h.last_ok) bits.push('<span class="tag">zuletzt ok ' + esc(h.last_ok) + '</span>');
    return '<div class="job"><div class="top">'
      + '<div style="display:flex;align-items:center"><span class="dot ' + h.state + '"></span>'
      + '<div><div class="ti">' + esc(h.source) + '</div>'
      + '<div class="co" style="text-transform:none">' + esc(h.kind) + '</div></div></div>'
      + '</div><div class="tags">' + bits.join('') + '</div>'
      + (h.error ? '<div class="why">' + esc(h.error) + '</div>' : '')
      + '</div>';
  }}).join('');
}}
...
function draw() {{
  ...
  if (view === 'jobs') render(activeFilter);
  else if (view === 'quellen') renderHealth();
  else renderEmployers();
}}
...
def render_xlsx(jobs: list[dict], out: str = "docs/praktika.xlsx",
    ...
    headers = ["Neu", "Score", "Unternehmen", "Rolle", "Standort", "Monate",
               "Start", "Sektor", "Quelle", "Ausgeschrieben", "Zuerst gesehen",
               "Status", "Notiz", "Link"]
    ws.append(headers)
    ...
    for j in jobs:
        ...
        ws.append([
            ...
            j.get("start_date") or "",
            j.get("sector") or "",
            j.get("source") or "Quelle nicht vermerkt",
            (j.get("posted") or "").strip() or "nicht auffindbar",
    ...
    widths = [6, 7, 20, 52, 22, 8, 12, 12, 24, 15, 14, 12, 30, 60]
    ...
    # Drittes Blatt: Quellenampel
    ws3 = wb.create_sheet("Quellen")
    h3 = ["Ampel", "Quelle", "Art", "Stellen zuletzt", "Laeufe still",
          "Zuletzt ok", "Letzter Fehler"]
    ws3.append(h3)
    ...
    fills = {"rot": PatternFill("solid", fgColor="F8D7DA"),
             "gelb": PatternFill("solid", fgColor="FFF3CD"),
             "gruen": PatternFill("solid", fgColor="D8F3DC")}
    for h in sorted(health or [], key=lambda x: -(x.get("quiet_runs") or 0)):
        state = _ampel(h)
        ws3.append([
            state,
            h.get("source", ""),
            h.get("kind", ""),
            h.get("last_count") or 0,
            h.get("quiet_runs") or 0,
            (h.get("last_ok") or "")[:10],
            (h.get("last_error") or "")[:200],
        ])
        ws3.cell(row=ws3.max_row, column=1).fill = fills[state]

    for i, w in enumerate([9, 40, 15, 15, 13, 13, 70], start=1):
