...
Drei Stufen:
  1  Entdeckung  - breite Suche ueber Adzuna, findet unbekannte Firmen
...
def main() -> int:
    ...
    disc_jobs, disc_errors, health = run_discovery(disc_cfg)
    ...
    for company in companies_cfg["companies"]:
        ...
        if company.get("enabled") is False:
            log.info("%-28s uebersprungen (enabled: false)", name[:28])
            continue
        ...
        try:
            ...
            health.append({"source": f"Karriereseite {name}", "kind": "karriereseite",
                           "count": 0, "error": f"{type(e).__name__}: {str(e)[:180]}"})
            continue

        if not jobs and company.get("status") != "auto":
            broken.append(company)

        # Nachweis: jede Stelle weiss, von welcher Karriereseite sie stammt.
        for j in jobs:
            if not j.source:
                j.source = f"Karriereseite {name}"
    ...
    if broken:
        ...
        for company in broken:
            # 'hints' aus companies.yaml werden zuerst geprueft - aber nur
            # uebernommen, wenn sie tatsaechlich Stellen liefern.
            # Im Tageslauf bewusst knapp gehalten (Zeitbudget des Workflows).
            # Die gruendliche Suche macht der Probe-Lauf.
            found = detect(company["name"], hints=company.get("hints"), max_checks=40)
            if not found:
    ...
    render_html(active, errors, employers=employer_rows, health=health_rows)
    render_xlsx(active, employers=employer_rows, health=health_rows)

    quiet = [h for h in health_rows if (h.get("quiet_runs") or 0) >= 7]
    ...
    if quiet:
        log.info("")
        log.info("STILLE QUELLEN - laufen ohne Fehler, liefern aber seit >=7 Laeufen nichts:")
        for h in quiet:
            log.info("  %-42s %d Laeufe still", h["source"][:42], h["quiet_runs"])
